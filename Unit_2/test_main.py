...
videoSta=0   #2:不调用API  3：调用API   一次
getAPISta=0    #4：不更新GUI  5：更新GUI
...
def cezhong(xing1,xing2):
    ...
    while True:
        if tizhongSta==0:
            ...
            nowTime=time.time()
            if sums>0.6 and nowTime-startime>5:
...
def opWi():
    ...
    if needAPI=='success':
            ...
            lab2 = Label(root, text="  序  号：   1    ", font=('Times', 50, ''))
            lab3 = Label(root, text="", font=('Times', 20, ''))

            lab4 = Label(root, textvariable= lname, font=('Times', 50, ''))
            lab5 = Label(root, text="", font=('Times', 20, ''))

            lab6 = Label(root, textvariable = ltype, font=('Times', 50, ''))#yuan lai :number
            lab7 = Label(root, text="", font=('Times', 20, ''))
            lab8 = Label(root, text="分类情况：待 分 类", font=('Times', 50, ''))
            lab1.pack()
            lab2.pack()
            lab3.pack()
            lab4.pack()
